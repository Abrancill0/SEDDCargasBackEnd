...
{
    public class UnidadEvaluacionController : ApiController
    {
        ...
        public JObject Post(ParametorsEntrada Datos)
        {
            ...
            try
            {
                ...
                for (int i = 1; i < ArregloFinal.Length; i++)
                {
                    ...
                    string NombreUnidadEvaluacion = Convert.ToString(Valores[1]);
                    string Idioma = Convert.ToString(Valores[2]);
                  
                    SqlCommand comando2 = new SqlCommand("Cargas.AltaUnidadEvaluacion");
                    comando2.CommandType = CommandType.StoredProcedure;

                    //Declaracion de parametros 
                    comando2.Parameters.Add("@Empresa", SqlDbType.VarChar);
                    comando2.Parameters.Add("@NombreUnidadEvaluacion", SqlDbType.VarChar);
                    comando2.Parameters.Add("@Idioma", SqlDbType.VarChar);

                    //Asignacion de valores a parametros
                    comando2.Parameters["@Empresa"].Value = Empresa;
                    comando2.Parameters["@NombreUnidadEvaluacion"].Value = NombreUnidadEvaluacion;
